...
$(document).ready(function() {
    ...
    $('#changeNetwork').on('click', function() {
        showModal('Operation not Available', 'This operation is unavailable until Celo Donut Fork.')
    });
    updateTokenListTab();
...
async function handleAcceptedCrossTransferEvent(event) {
    ...
    const txDataHash = await bridgeContract.methods.getTransactionDataHash(
        _to, 
        _amount, 
        _blockHash, 
        _transactionHash, 
    ...
    .then(async (balance) => {
        ...
        return new Promise((resolve, reject) => {
            bridgeContract.methods
                .receiveTokensTo(31, tokenContract.options.address, hathorAddress, amountBN.toString())
                .send({from: address, gasPrice:gasPrice, gas:200_000}, async (err, txHash) => {
                    console.log(err)
                    console.log(txHash)
                if (err) return reject(err);
                try {
...
async function updateNetwork(newNetwork) {
    ...
    try {
        ...
        if(config == null) {
            ...
            throw new Error(`Wrong Network.<br /> Please connect your wallet to <b>${isTestnet ? 'Sepolia' : 'Ethereum Mainnet'}</b>`);
        }
...
function updateTokenListTab() {
    ...
    for(let aToken of TOKENS) {
        if(aToken[htrConfig.networkId] != undefined) {
            ...
            tabHtml += `\n          <a href="${htrConfig.explorer}/address/${aToken[htrConfig.networkId].address.toLowerCase()}" class="address" target="_blank">`;
            tabHtml += `\n            <span><img src="${aToken.icon}" class="token-logo"></span>${aToken[htrConfig.networkId].symbol}`;
            tabHtml += `\n          </a>`;
            tabHtml += `\n       </div>`;
            tabHtml += `\n       <div class="col-4">`;
            tabHtml += `\n           <button class="copy btn btn-outline-secondary" type="button" data-clipboard-text="${aToken[htrConfig.networkId].address.toLowerCase()}" data-toggle="tooltip" data-placement="bottom" title="Copy token address to clipboard">`;
            ...
            tabHtml += `\n    <div class="col-5 row">`;
            tabHtml += `\n      <div class="col-8 font-weight-bold">`;
            tabHtml += `\n          <a href="${htrConfig.crossToNetwork.explorer}/address/${aToken[htrConfig.crossToNetwork.networkId].address.toLowerCase()}" class="address" target="_blank">`;
            tabHtml += `\n              <span><img src="${aToken.icon}" class="token-logo"></span>${aToken[htrConfig.crossToNetwork.networkId].symbol}`;
            tabHtml += `\n          </a>`;
            tabHtml += `\n      </div>`;
            tabHtml += `\n      <div class="col-4">`;
            tabHtml += `\n          <button class="copy btn btn-outline-secondary" type="button" data-clipboard-text="${aToken[htrConfig.crossToNetwork.networkId].address.toLowerCase()}" data-toggle="tooltip" data-placement="bottom" title="Copy the address">`;
            tabHtml += `\n              <i class="far fa-copy"></i>`;
            tabHtml += `\n          </button>`;
            tabHtml += `\n      </div>`;
            tabHtml += `\n    </div>`;
